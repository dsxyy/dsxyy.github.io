...
- 渲染器：仓库内嵌 MDwiki，入口为 `index.html`，导航和首页内容在 `index.md`。
...
在 `index.md` 的 `Demo` 列表中增加一个相对链接：

```markdown
- [文章标题](topic-slug.md)
```

专题文章使用包含目录的相对路径。保持周围文件原有换行风格；只增加必要条目，不重排历史链接。
...
git status --short
git diff --check
git diff -- <article-path> index.md
```

只暂存明确文件：

```bash
git add -- <article-path> index.md
```
